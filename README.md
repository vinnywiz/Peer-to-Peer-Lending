...
Listings with bad loan Status using a scatterplot. In yellow it shows the customers who have a bad loan and they represent a very small population size. 
...
To be able to understand and improve our model’s performance, we established a baseline for the data that we have. We employed a **dummy classifier** as our baseline model. This classifier serves as a simple baseline to compare against other more complex classifiers. The other models we were interested in testing for this problem were **Logistic Regression, Random Forest, and XGBoost classifier.** For our Random Forest and XGBoost classifier, we used grid search to get the optimized values of hyperparameters which were included into the model as defaults. We observed the performance of our models through the model classification report (our interest was Accuracy, Precision, Recall, and F1-score). We primarily care about correctly identifying **bad loans**, so the **recall score** will be necessary. 
